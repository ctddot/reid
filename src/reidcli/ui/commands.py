...
from reidcli.personas import Persona
...
MODEL_CATALOG: tuple[tuple[str, str, str], ...] = (
    ("reidverse", "gpt-5-4", "default ReidVerse model"),
    ("openai", "gpt-5", "OpenAI family"),
    ("openai", "gpt-5-mini", "OpenAI family"),
    ("openai", "gpt-4.1", "OpenAI family"),
    ...
    ("groq", "moonshotai/kimi-k2-instruct", "Kimi on Groq"),
    ("moonshot", "kimi-k2-0711-preview", "Kimi"),
    ("ollama", "llama3.2", "local Ollama"),
    ("ollama", "qwen2.5-coder", "local Ollama"),
)
...
SLASH_COMMANDS: list[tuple[str, str, str, str]] = [
    ...
    ("/persona", "<list|add|use|show|delete> ...", "manage built-in and custom personas", "Config & Policy"),
...
def _handle_persona(orchestrator: Orchestrator, arg: str) -> None:
    parts = _split_args(arg)
    if parts is None:
        return
    if not parts or parts[0] == "list":
        table = Table(title="personas", box=BOX, show_header=True, header_style=f"bold {PRIMARY}", border_style=PRIMARY)
        table.add_column("active", width=6)
        table.add_column("name", style="bold")
        table.add_column("category")
        table.add_column("source", style="dim")
        active = orchestrator.active_persona.name if orchestrator.active_persona else ""
        for persona in orchestrator.persona_store.list():
            table.add_row("●" if persona.name == active else "", persona.name, persona.category, "built-in" if persona.builtin else "user")
        render.console.print(table)
        return
    action = parts[0]
    if action == "add":
        if len(parts) < 4:
            render.print_error("usage: /persona add <name> <universal|model-or-provider> <prompt>")
            return
        name = parts[1].strip().lower()
        category = parts[2].strip().lower()
        prompt = " ".join(parts[3:]).strip()
        if not name or not prompt:
            render.print_error("persona name and prompt are required")
            return
        try:
            orchestrator.persona_store.save(Persona(name=name, category=category or "universal", prompt=prompt))
        except ValueError as exc:
            render.print_error(str(exc))
            return
        render.print_info(f"persona saved → {name} ({category or 'universal'})")
        return
    if action == "use":
        if len(parts) != 2:
            render.print_error("usage: /persona use <name|off>")
            return
        try:
            orchestrator.set_persona(parts[1])
        except KeyError as exc:
            render.print_error(str(exc))
            return
        active = orchestrator.active_persona.name if orchestrator.active_persona else "off"
        render.print_info(f"persona → {active}")
        return
    if action == "show":
        if len(parts) != 2:
            render.print_error("usage: /persona show <name>")
            return
        persona = orchestrator.persona_store.get(parts[1])
        if persona is None:
            render.print_error(f"persona not found: {parts[1]}")
            return
        render.console.print(
            Panel(
                Text(persona.prompt or "(empty)", style="white"),
                title=f"{persona.name} · {persona.category}",
    ...
    if action == "delete":
        if len(parts) != 2:
            render.print_error("usage: /persona delete <name>")
            return
        try:
            deleted = orchestrator.persona_store.delete(parts[1])
        except ValueError as exc:
            render.print_error(str(exc))
            return
        if not deleted:
            render.print_error(f"persona not found: {parts[1]}")
            return
        if orchestrator.active_persona and orchestrator.active_persona.name == parts[1].strip().lower():
            orchestrator.set_persona("")
        render.print_info(f"persona deleted → {parts[1]}")
        return
    render.print_error("usage: /persona <list|add|use|show|delete> ...")
...
def _handle_models(orchestrator: Orchestrator) -> None:
    ...
    for name in names:
        ...
        cfg = orchestrator.config.providers.get(name)
        model = (cfg.default_model if cfg and cfg.default_model else "") or getattr(provider, "default_model", "")
        key = (name, model or "-")
        seen.add(key)
        marker = "●" if name == active_provider and (not active_model or model == active_model) else ""
        table.add_row(marker, name, model or "-", "connected provider")

    for provider, model, source in MODEL_CATALOG:
        key = (provider, model)
        if key in seen:
            continue
        marker = "●" if provider == active_provider and model == active_model else ""
        table.add_row(marker, provider, model, source)

    render.print_info("use /model <name> to set the active session model; use /connect for provider endpoints")
    render.console.print(table)
...
def handle(orchestrator: Orchestrator, line: str) -> str:
    ...
    if cmd in ("help", "?"):
        ...
        _handle_persona(orchestrator, arg)
