...
The default path uses native terminal scrollback with a small prompt_toolkit
input app for the red prompt box and slash-command menu. The older full-screen
prompt_toolkit implementation remains below as a fallback/experiment, but
`run()` routes to the native terminal UI.
...
class SlashCommandCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):  # type: ignore[no-untyped-def]
        ...
        arg_completions = {
            ...
            "/nyx ": ["on", "off"],
            "/persona use ": ["off"] + [p.name for p in self.orchestrator.persona_store.list()],
            "/persona show ": [p.name for p in self.orchestrator.persona_store.list()],
            "/persona delete ": [p.name for p in self.orchestrator.persona_store.list() if not p.builtin],
            "/persona add ": ["writer universal ", "coder universal ", "reviewer universal "],
            "/persona ": ["list", "add", "use", "show", "delete"],
            "/plan ": ["on", "off"],
            "/web ": ["on", "off"],
...
class _DarkCompletionsControl(UIControl):
    ...
    def preferred_height(self, width: int, max_available_height: int, wrap_lines: bool, get_line_prefix) -> int | None:  # type: ignore[no-untyped-def]
        state = get_app().current_buffer.complete_state
        return min(max_available_height, 18, len(state.completions)) if state else 0
    ...
    def create_content(self, width: int, height: int) -> UIContent:
        ...
        if not completions:
            return UIContent()
        index = min(max(state.complete_index or 0, 0), len(completions) - 1)
        command_width = max(get_cwidth(c.display_text) for c in completions) + 4
        left_width = min(max(18, command_width), max(18, min(36, width // 2)))
        ...
        def get_line(i: int):
            ...
            command_style = "#ff8a8a bold" if current else "#ff5f5f"
            meta_style = "#e6e6e6" if current else "#8a8a8a"
            left = self._fit(marker + completion.display_text, left_width)
            right = self._fit(completion.display_meta_text, meta_width)
            return [(command_style, left), (meta_style, right)]
...
class _DarkCompletionsMenu(ConditionalContainer):
    def __init__(self, width: int | None = None) -> None:
        menu_width = Dimension.exact(width) if width is not None else Dimension(weight=1)
        super().__init__(
            content=Window(
...
class ChatApp:
    ...
    def __init__(self, orchestrator: Orchestrator, initial_prompt: str | None = None) -> None:
        ...
        self._thinking = {"flag": False, "start": 0.0, "gerund": "", "last_swap": 0.0, "activity": ""}
    ...
    def _build_status_fragments(self):  # type: ignore[no-untyped-def]
        ...
        frags = [
            ...
            (_interaction_mode_color(self.orchestrator), _interaction_mode_label(self.orchestrator)), sep,
            ("#9e9e9e", usage), sep,
    ...
    def _build_spinner_fragments(self):  # type: ignore[no-untyped-def]
        ...
        if self._show_activity and self._thinking.get("activity"):
            frags.extend([("#9e9e9e", " · "), ("#9e9e9e", str(self._thinking["activity"]))])
        return frags
    ...
    def _build_layout(self) -> Layout:
        ...
        floated = FloatContainer(
            content=root,
            floats=[Float(left=0, ycursor=True, content=_DarkCompletionsMenu())],
        )
    ...
    def _build_key_bindings(self) -> KeyBindings:
        ...
        @kb.add("s-tab", filter=~is_thinking & ~is_approving)
        def _cycle_interaction_mode(event) -> None:  # type: ignore[no-untyped-def]
            self.orchestrator.cycle_interaction_mode()
            self.app.invalidate()

        @kb.add("c-b")
        def _toggle_activity(event) -> None:  # type: ignore[no-untyped-def]
            self._show_activity = not self._show_activity
            self.app.invalidate()

        @kb.add("up", filter=has_completions & ~is_thinking & ~is_approving)
        def _complete_previous(event) -> None:  # type: ignore[no-untyped-def]
            event.current_buffer.complete_previous()

        @kb.add("down", filter=has_completions & ~is_thinking & ~is_approving)
        def _complete_next(event) -> None:  # type: ignore[no-untyped-def]
    ...
    def _box_color(self) -> str:
        ...
        if self._deepread_prefix_len():
            ...
            return _DEEPREID_PULSE_SHADES[idx]
        return _interaction_mode_color(self.orchestrator)
    ...
    async def _submit_text(self, text: str) -> None:
        ...
        self._thinking["activity"] = "starting"
        self._cancel_confirm_until = 0.0
        self.app.invalidate()
...
def _interaction_mode(orchestrator: Orchestrator) -> str:
    return getattr(orchestrator, "interaction_mode", "accept_edits")


def _interaction_mode_color(orchestrator: Orchestrator) -> str:
    return _INTERACTION_MODE_COLORS.get(_interaction_mode(orchestrator), _BOX_COLOR_NORMAL)


def _interaction_mode_label(orchestrator: Orchestrator) -> str:
    return _INTERACTION_MODE_LABELS.get(_interaction_mode(orchestrator), _interaction_mode(orchestrator))
...
def _terminal_header(orchestrator: Orchestrator) -> None:
    ...
    render.console.print(
        Text.assemble(
            ...
            ("  ·  ", DIM),
            (_interaction_mode_label(orchestrator), _interaction_mode_color(orchestrator)),
        )
    )
    render.console.print()
...
def _poll_terminal_keys() -> tuple[bool, bool, str]:
    if sys.platform != "win32":
        return False, False, ""
    try:
        import msvcrt
    ...
    while msvcrt.kbhit():
        ...
        if ch == b"\x02":
            ...
            try:
                other += ch.decode(errors="ignore")
            except UnicodeDecodeError:
                pass
    return ctrl_b, esc, other
...
def _thinking_line(
    ...
    *,
    ...
    if stopping:
        ...
        detail = activity if show_thinking and activity else "ctrl+b details"
    return (
        f"\r\x1b[2K    {logo} \x1b[{color}mThinking\x1b[0m"
...
def _run_turn_with_thinking(orchestrator: Orchestrator, text: str) -> tuple[dict, int, bool]:
    ...
    show_thinking = False
    activity_state: dict[str, str] = {"text": "starting"}
    ...
    def _activity(text: str) -> None:
        with activity_lock:
            activity_state["text"] = text

    def _latest_activity() -> str:
        with activity_lock:
            return activity_state["text"]

    def _worker() -> None:
        try:
            holder["result"] = orchestrator.submit_task(
                text,
                approver=_terminal_approve,
                cancel=cancel_event.is_set,
                activity=_activity,
            )
    ...
    while thread.is_alive():
        ctrl_b, esc, _other = _poll_terminal_keys()
        if ctrl_b:
...
def _terminal_read_input(orchestrator: Orchestrator, history: InMemoryHistory, status_line: str | None) -> str:
    ...
    @kb.add("s-tab")
    def _cycle_interaction_mode(event) -> None:  # type: ignore[no-untyped-def]
        orchestrator.cycle_interaction_mode()
        event.app.invalidate()

    def _border_line(left: str, fill: str, right: str) -> list[tuple[str, str]]:
        current_width = _terminal_box_width()
        return [(_interaction_mode_color(orchestrator), left + (fill * max(0, current_width - 2)) + right)]

    top = Window(
        FormattedTextControl(lambda: _border_line("╭", "─", "╮")),
        ...
        [
            Window(
                FormattedTextControl(
                    lambda: [
                        (_interaction_mode_color(orchestrator), "│"),
                        (f"{_interaction_mode_color(orchestrator)} bold", " You"),
                        ("#8a8a8a", " > "),
                    ]
                ),
            ...
            Window(
                FormattedTextControl(lambda: [(_interaction_mode_color(orchestrator), "│")]),
    ...
    root = FloatContainer(
        content=HSplit(content, width=Dimension(weight=1)),
        floats=[Float(left=0, ycursor=True, content=_DarkCompletionsMenu())],
    )
    app = Application(
