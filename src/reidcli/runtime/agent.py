...
def _tool_activity(name: str, args: dict[str, Any], phase: str = "") -> str:
    target = str(args.get("path") or args.get("cwd") or args.get("pattern") or "").strip()
    if name == "read_file":
        action = "reading file"
        ...
        target = str(args.get("command") or "").strip()
    elif name == "web_search":
        action = "searching web"
        target = str(args.get("query") or "").strip()
    elif name == "spawn_agent":
        action = "running subagent"
        target = str(args.get("name") or args.get("prompt") or "").strip()
    else:
        action = f"using {name}"
    if phase:
        action = f"{action} {phase}"
    if len(target) > 80:
        target = target[:77] + "..."
        ...
        for _step in range(max_steps):
            ...
            for call in resp.tool_calls:
                ...
                if call.name in disabled_tools:
                    ...
                    if activity is not None:
                        activity(_tool_activity(call.name, call.arguments))
                    result = self.tools.dispatch(call.name, call.arguments, ctx)
                if activity is not None:
                    activity(_tool_activity(call.name, call.arguments, "done" if result.ok else "failed"))
                tool_log.append(
