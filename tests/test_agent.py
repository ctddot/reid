...
def test_activity_callback_reports_tool_work(tmp_path: Path) -> None:
    agent = _agent(tmp_path)
    state = _state(tmp_path)
    events: list[str] = []
    agent.run_turn(state, "list the current dir", activity=events.append)

    assert "waiting for model" in events
    assert any("searching directory" in event for event in events)
    assert any("searching directory done" in event for event in events)
