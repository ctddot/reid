...
def test_models_catalog_includes_non_gpt_families() -> None:
    from reidcli.ui.commands import MODEL_CATALOG

    models = {model for _provider, model, _source in MODEL_CATALOG}
    assert "gemini-3-flash" in models
    assert "llama-3.3-70b-versatile" in models
    assert "kimi-k2-0711-preview" in models
    assert "claude-sonnet-4-20250514" in models


def test_persona_add_and_use(tmp_path: Path) -> None:
    orch = _orch(tmp_path)
    assert handle(orch, "/persona add helper universal reply like a careful helper") == "continue"
    assert (tmp_path / "personas.json").exists()
    assert handle(orch, "/persona use helper") == "continue"
    assert orch.active_persona is not None
    assert orch.active_persona.name == "helper"
    assert "careful helper" in orch.agent.base_system_prompt
    assert handle(orch, "/persona use off") == "continue"
    assert orch.active_persona is None


def test_interaction_mode_cycle_applies_tool_permissions(tmp_path: Path) -> None:
    orch = _orch(tmp_path)
    assert orch.interaction_mode == "accept_edits"
    assert orch.cycle_interaction_mode() == "automation"
    assert orch.agent.context_extras["automation_enabled"] is True

    assert orch.cycle_interaction_mode() == "plan"
    disabled = orch.agent.context_extras["disabled_tools"]
    assert {"write_file", "patch_file", "run_command"} <= disabled
    assert orch.agent.context_extras["plan_enabled"] is True

    assert orch.cycle_interaction_mode() == "accept_edits"
    assert orch.agent.context_extras["disabled_tools"] == set()
    assert orch.agent.context_extras["plan_enabled"] is False
